#!/bin/zsh
# OSINT Tools Setup + Updater + Validator
...
Workspaces:
- Outputs in  ~/osint-tools-workspaces/<target>/<timestamp>/
...
validator() {
  ...
  dpkg -s debian-archive-keyring >/dev/null 2>&1 && ok "Debian archive keyring package present" || warn "Debian archive keyring package missing"
  ...
  local WS="${REAL_HOME}/osint-tools-workspaces"
...
main() {
  ...
  log "==== OSINT Tools Setup starting ===="
  apt_self_heal
  install_base_packages
  setup_python_envs
  setup_go_env
  ...
  run "${SUDO} -u \"$TARGET_USER\" mkdir -p \"$TARGET_HOME/osint-tools-workspaces\""
  post_install_checks
  usage_hints
